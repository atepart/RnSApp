...
1) **Ввод данных (таблица «Данные»)**  
   - Столбцы: `№`, `Имя`, чекбокс выбора, `Диаметр`, `Rₙ (Ω)`, `RnS`, `Ошибка RnS`, `Суммарный уход`, `RnS/Rn`, `1/√Rₙ`.  
...
### Как работает взвешенный линейный fit

Для расчёта параметров приложение строит прямую
`Y = slope · D + intercept`, где `D` — диаметр, а
`Y = 1 / √(Rn + Rпосл)`. Точки учитываются не одинаково: для каждой выбранной
строки с положительным диаметром используется множитель остатка `q = 1 / D`.
Поэтому фактический вес точки в методе наименьших квадратов равен
`w = q² = 1 / D²`:

`Σ [1 / Dᵢ² · (Yᵢ − (slope · Dᵢ + intercept))²] → min`.

Это означает, что измерения с меньшим диаметром сильнее влияют на положение
прямой. Например, при `D = 1, 2, 4, 8` фактические веса равны соответственно
`1`, `0,25`, `0,0625` и `0,015625`. Такая схема соответствует предположению,
что абсолютная погрешность `Y` растёт примерно пропорционально диаметру.

В XLSX столбец **«Вес (1/D)»** показывает именно множитель `q`, а не
квадрат веса. Excel рассчитывает тот же взвешенный fit через пять скрытых
вспомогательных сумм (`Σw`, `ΣwD`, `ΣwY`, `ΣwD²`, `ΣwDY`), а линия на графике
строится по полученным `slope` и `intercept`. Обычная линия тренда Excel не
используется, потому что она выполняет невзвешенную регрессию.

В fit попадают только выбранные строки с числовым `Y` и диаметром `D > 0`.
Пустые и некорректные строки исключаются. Если физическая модель погрешности
измерений изменится, правило весов также нужно пересмотреть.
...
3) **Работа с 16 ячейками (блок «Запись»)**  
   ...
   - Контекстное меню ячейки (ПКМ): «Показать данные» (загрузить сохранённые значения обратно в таблицы/график), «Переименовать», «Перезаписать», «Очистить» (полностью удалить сохранённые данные ячейки).  
